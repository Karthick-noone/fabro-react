...
const ImageGallery = () => {
  ...
  useEffect(() => {
    const fetchImageUrls = async () => {
      try {
        const roomType = "double"; // Change room type here
  ...
  const handleToggle = () => {
    Swal.fire({
      title: "Are you sure?",
      text: `Do you want to set the status to ${
  ...
  useEffect(() => {
    const fetchStatus = async () => {
      try {
        const roomId = 3; // Replace with the actual room ID
        const roomType = "double"; // Room type
        const response = await axios.get(
          `${BASE_URL}/api/getStatus/${roomId}?roomType=${roomType}`
        );
          ...
          >
            ...
            <Dropdown className="mt-3">
              ...
              <Dropdown.Menu className="custom-dropdown-menu ml-2">
                ...
                <Link to={`/Admin/DeleteImage?roomType=double`}>
