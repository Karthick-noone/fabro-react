...
const BookingForm = () => {
  ...
  useEffect(() => {
    const fetchRoomLimits = async () => {
      try {
        const response = await fetch(`${BASE_URL}/api/roomLimits`);
  ...
  <center>
    <h2 style={{ marginTop: "-50px" }}>Available Rooms</h2>
  </center>
  ...
  <div
    style={{
      ...
      marginTop: "20px",
      overflowX: "auto", // Add overflowX auto for horizontal scrolling
      maxWidth: "100%", // Ensure the content fits within the viewport
    }}
  >
    {cardData.map((card) => (
      <div
      ...
      >
        <div>
          <h3 style={{ color: "#101111", fontSize: "20px" }}>{card.title}</h3>
          <p style={{ color: "#101111" }}>{`Available: ${
            roomLimits[card.id] || 0
          }`}</p>
        </div>
        <p style={{ color: "#101111" }}>{`Price: Rs.${card.price}/pernight`}</p>
        <div
          style={{
            display: "flex",
            alignItems: "center",
    ...
    <div className="col-lg-2">
      <div style={{ marginTop: "20px", textAlign: "center" }}>
        ...
        >
          ...
          <h4>Total Price</h4>
          <p style={{ color: "#101111" }}>{`Rs. ${displayData.totalPrice}`}</p>
        </div>
      </div>
    </div>
  </div>
